...
typedef struct {
    ...
    // ZFF Config
    float zff_trend_window_ms;    // Window for removing low-frequency trend (default: 10.0)
    ...
    // Detection Logic
    float min_syllable_dist_ms;   // Minimum distance between syllables (default: 100.0)
    float threshold_peak_rate;    // Absolute floor for peakRate threshold (default: 0.0003)
    float adaptive_peak_rate_k;   // Adaptive threshold = mean + k*std (0 disables, default: 4.0)
    float adaptive_peak_rate_tau_ms; // Time constant for adaptive stats (default: 500.0)
...
// Returns the number of events detected in this block (that are now "ready" after context delay)
// Populates 'events_out' up to 'max_events' capacity.
SYLLABLE_API int syllable_process(SyllableDetector* detector, 
                                  const float* input, 
                                  int num_samples, 
                                  SyllableEvent* events_out, 
                                  int max_events);
