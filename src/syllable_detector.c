...
struct SyllableDetector {
  ...
  int peak_sample_offset; // Samples from onset to peak (for pr_slope)
...
SyllableConfig syllable_default_config(int sample_rate) {
  ...
  cfg.min_syllable_dist_ms = 100.0f; // Reverted to original
  cfg.threshold_peak_rate = 0.0003f; // Reverted to original
  cfg.adaptive_peak_rate_k = 4.0f;   // Reverted to original
  cfg.adaptive_peak_rate_tau_ms = 500.0f;
  cfg.voiced_hold_ms = 30.0f;
  cfg.hysteresis_on_factor = 1.2f;  // Slightly stricter ON (reduced from 1.3)
  cfg.hysteresis_off_factor = 0.8f; // Keep detections stable
  cfg.context_size = 2;
...
static float calculate_prominence(SyllableDetector *d, int target_idx) {
  // Enhanced prominence: Compare Energy, PeakRate, Duration, F0 to neighbors
  BufferedEvent *target = &d->event_buffer[target_idx];
  if (!target->is_ready)
    return 0.0f;
  ...
  // Calculate individual ratio scores (>1.0 means more prominent)
  ...
  // F0 change bonus: positive delta_f0 (higher pitch) indicates emphasis
  // Normalize delta_f0 to roughly 0-1 range (assuming typical delta of 0-50Hz)
  float f0_bonus =
  ...
  // Energy(0.2) + PeakRate(0.2) + Duration(0.2) + PR_slope(0.2) + F0(0.2)
  float score = 0.2f * e_score + 0.2f * pr_score + 0.2f * d_score +
                0.2f * slope_score + 0.2f * (1.0f + f0_bonus);
...
int syllable_process(SyllableDetector *d, const float *input, int num_samples,
  ...
  for (int i = 0; i < num_samples; i++) {
    ...
    if (d->state == STATE_IDLE) {
      // Trigger Condition: Rising PeakRate > ON threshold AND Voiced
      if (peak_rate > threshold_on && d->is_voiced) {
      ...
      if (peak_rate > d->max_peak_rate_in_syllable) {
        d->max_peak_rate_in_syllable = peak_rate;
        d->wip_event.peak_rate = peak_rate;
        d->peak_sample_offset = d->state_timer; // Record time to peak
      }
      ...
      if (peak_rate < d->max_peak_rate_in_syllable * 0.5f) {
        ...
        // Calculate PR_slope: peak_rate / rise_time
        float rise_time_s =
    ...
    while (d->buf_count > context_needed && events_written < max_events) {
      ...
      // Calculate delta_f0 from context before prominence scoring
...
int syllable_flush(SyllableDetector *d, SyllableEvent *events_out,
  ...
  while (d->buf_count > 0 && events_written < max_events) {
    ...
    // Calculate delta_f0 before prominence scoring
    calculate_delta_f0(d, d->buf_read_idx);
